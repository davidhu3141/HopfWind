...
class SpecEntity extends VisualizerOrthogonal {
    ...
    applySettingForWPE(properties) {
        ...
        if (properties.pixelated) {
            this.pixsz = properties.pixelated.value * 2
            this.windowResized()
        }
    ...
    render(time, audioSamples) {
        ...
        // this.scene.position.y = Math.sin(time * 0.015) * 5
...
void main() {
    ${defineTex0}
    vec2 centered = (vUV - vec2(.5,.5)) * 90.;
    // centered[0] = centered[0] * 2.;
    float r = length(centered);
    float theta = atan(centered[1], centered[0]);
    float x = centered[0];
    float y = centered[1];

    float t = (cos(centered[0]) + cos(centered[1]) + 4.) / 8.;
    t = pow(t, .051);
    vec2 mv1 = -(vec2(-sin(x), -sin(y))); // tried y no -
    vec2 mv2 = vec2(sin(y), -sin(x));// + mv1;
    // vec2 mv = mv1 * (1.-t) + mv2 * (t); // exchange t
    // vec2 mv = vec2(x-y, x+y) * .05;
    vec2 mv = vec2(-y+x, x+y) * .05;



    vec2 vUV2 = vUV - moveVelocityX * mv;

    // vec2 vUV2 = vUV - moveVelocityX * (abs(sin(r*20.0))*5.0+0.02) * centered / r;
    // vec2 vUV2 = vUV - moveVelocityX*vec2(pow(sin(60.*(vUV[1]+vUV[0])),1.0), pow(cos(60.*(vUV[1]-vUV[0])),1.0));
    // vec2 vUV2 = vUV - moveVelocityX*vec2(
    //     pow(cos(100.*(vUV[0])),1.0),    
    //     -pow(sin(100.*(vUV[1])),1.0)
    // );
    // vec2 vUV2 = vUV - moveVelocityX*vec2(
    //     pow(-sin(60.*(vUV[0]))*sin(60.*(vUV[1])),1.0),
    //     -pow(cos(60.*(vUV[0]))*cos(60.*(vUV[1])),1.0)
    // );
    // vec2 vUV2 = vUV - moveVelocityX*vec2(
    //     -pow(cos(100.*(vUV[1])),1.0)+1.0, 
    //     pow(cos(100.*(vUV[0])),1.0)+1.0);
